...
<div x-data="{ tab: 'upcoming', cancelModal: false, cancelId: null }">
    ...
    <div x-show="tab==='upcoming'">
        ...
        <div class="bg-white border border-gray-100 rounded-2xl py-20 text-center">
            <div class="w-14 h-14 bg-violet-50 rounded-2xl flex items-center justify-center mx-auto mb-4">
                <svg class="w-7 h-7 text-violet-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
            </div>
            <p class="text-gray-400 text-sm">No upcoming appointments</p>
            <a href="{{ route('appointments.create') }}" class="mt-2 inline-block text-violet-600 text-sm font-medium hover:underline">Book one →</a>
        </div>
        @else
    ...
    <!-- Cancel Modal -->
    <div x-show="cancelModal" x-transition class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
        <div @click.outside="cancelModal=false" class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            ...
            <form method="POST" :action="`/appointments/${cancelId}/cancel`">
                ...
                <div class="mt-4 flex gap-3">
                    <button type="submit" class="flex-1 bg-red-500 text-white py-2.5 rounded-xl text-sm font-semibold hover:bg-red-600 transition">Confirm Cancel</button>
                    <button type="button" @click="cancelModal=false" class="px-5 py-2.5 border border-gray-200 rounded-xl text-sm text-gray-500 hover:bg-gray-50 transition font-medium">Go Back</button>
                </div>
            </form>
        </div>
    </div>
